...
- Node.js 22 LTS is the project standard. Use `nvm use` (reads `.nvmrc`) to match the correct version.
...
- 1. Copy env and review
  - `cp env.example .env`
  - Defaults: Postgres `localhost:5433`, Redis `localhost:6380`
...
- 2. Start dependencies
  - `docker compose up -d`
  - Note: Redis is mapped to host port `6380`; Postgres to `5433`.
...
- Postgres port in use: edit `docker-compose.yml` to change `5433:5432` (or free the port) and update `DATABASE_URL` accordingly.
